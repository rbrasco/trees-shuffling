...
def tree_to_latex(tree, sort=False, size_f=(15, 10), labels=True, label_b=(3, (-2, 0)), tree_name=None):
    ...
    scaled_vertices = list(
        map(
            lambda x: (
                x[0],
                (round(x[1][0] * size_f[0], 2), round(x[1][1] * size_f[1], 2)),
            ),
            vertices,
        )
    )
    ...
    def get_label(name):
        ...
        return (
            f"({name.split('-')[0]}," + "\\text{ }" + f"{name.split('-')[1]})"
            if "-" in name
            else name
        )
...
def _recursive_add_edges(T, G):
    if T.root:
        s_n = f"root_{T.trunk}"
        t_n = f"{T.node}-{T.trunk}" if T.node else f"leaf_{T.trunk}"
    ...
    if T.node:
        ...
        for branch in T.branches.values():
            ...
            t_label = branch.node if branch.node else f"leaf_{branch.trunk}"
            if not has_node(G, t_n):
                G.add_vertex(t_n, label=t_label)
