...
def string_to_tree_space(string, operad, sort=True):
    ...
    for i, operation in enumerate(operations):
        ...
        operads.append(Tree(operation, trunk, branches, operad, not i))
...
def extract_info(item):
    ...
    return operation, trunk, branches.split(",") if branches else []


def sorted(operations):
    n = len(operations)
    items = list(map(lambda x: (extract_info(x), x), operations))
    i = 0
    while i < n:
        item1 = items[i]
