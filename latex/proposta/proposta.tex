...
\end{ex}
\begin{defi}
    La \emph{categor\'ia de \'arboles planares con ra\'iz} $\Omega_p$ es la subcategor\'ia completa$|$llena de la categor\'ia de op\'eradas coloreadas no-sim\'etricas cuyos objetos son $\Omega_p(T)$ para todo \'arbol $T$.
...
\begin{defi}
    Una \emph{funci\'on degenerativa} asociada al v\'ertice $v$ es una funci\'on $\sigma_v\colon T\to T \char`\\ v$ en $\Omega_p$, donde $T \char`\\ v$ es el \'arbol que se obtiene al cortar el v\'ertice $v$ y juntar las dos aristas adjuntas en una nueva arista $e$.

    Esta funci\'on env\'ia los colores $e_1$ y $e_2$ de $\Omega_p(T)$ al color $e$ de $\Omega_p(T\char`\\v)$ y env\'ia la operacion generativa $v$ a la operaci\'on identidad $id_e$, mientras que es la identidad para los colores y operaciones generativas restantes.
...
\begin{lema}
    Sea $A$ y $B$ dos \'arboles en $\Omega_p$, toda flecha $f\colon A \to B$ en $\Omega_p$ descompone, salvando isomorf\'ias, como
    $$
        \xymatrix{
            A \ar[rd]_{\sigma} \ar[r]^f
            &B \\
            &C \ar[u]_\partial
        }
    $$
    donde  $\sigma\colon A\to B$ es una composici\'on de funciones degenerativas y $\partial\colon C\to B$ es una composici\'on de caras.
\end{lema}
Demo: por hacer
