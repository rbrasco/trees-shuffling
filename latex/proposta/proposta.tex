...
\subsubsection{Degeneradas}
Sea $T$ un \'arbol planar con ra\'iz y $v$ un v\'ertice de valencia uno en $T$.
\begin{defi}
    Una \emph{degenerada} asociada al v\'ertice $v$ es una funci\'on $\sigma_v\colon T\to T \backslash  v$ en $\Omega_p$, donde $T \backslash  v$ es el \'arbol que se obtiene al cortar el v\'ertice $v$ y juntar las dos aristas adjuntas en una nueva arista $e$.
...
\begin{obs}
    Las caras y las degeneradas generan toda la categor\'ia $\Omega_p$.
\end{obs}
El siguiente lema es una generalizaci\'on hacia $\Omega_p$ del lema en la categor\'ia $\Delta$, diciendo que toda flecha en dicha categor\'ia se puede escribir como composici\'on de degeneradas seguidas por caras.
...
\begin{lema}
    ...
    $$
    donde  $\sigma\colon S\to H$ es una composici\'on de degeneradas y $\partial\colon H\to T$ es una composici\'on de caras.
\end{lema}
Demo: por hacer
...
\subsubsection*{Relaciones elementales de degeneradas}
Sea $\sigma_v \colon T\to T\backslash v$ y $\sigma_w \colon T\to T\backslash w$ dos degeneradas distintas de $T$.
Seguidamente tenemos las degeneradas $\sigma_v \colon T\backslash w \to (T\backslash w)\backslash v$ y $\sigma_w \colon T\backslash v \to (T\backslash v)\backslash w$. Observamos que $(T\backslash v)\backslash w = (T\backslash w)\backslash v$, entonces el siguiente diagrama conmuta:
$$
    \xymatrix{
...
Sea $\sigma_v\colon T\to T\backslash v$ una degenerada y $\partial\colon T' \to T$ es una cara interna en una arista adjunta a $v$ o una cara externa en $v$, si es posible. Entonces, tenemos que $T'=T\backslash v$ y la composici\'on
...
\end{defi}
\begin{obs}
    Los morfismos de la categor\'ia $\Omega$ son generados por las caras y las degeneradas, an\'alogas al caso planar, y las isomorf\'ias no-planares.
\end{obs}
\begin{lema}
    ...
    $$
    donde  $\sigma\colon S\to S'$ es una composici\'on de degeneradas, $\varphi\colon S'\to T'$ es un isomorf\'ia, y $\partial\colon T\to T'$ es una composici\'on de caras.
\end{lema}
Demo: por hacer
...
\end{defi}
\begin{defi}
    Un dendrex $x\in X_T$ se llama \emph{degenerado} si existe otro dendrex $y\in X_S$ y una degenerada $\sigma\colon T\to S$ tal que $\sigma^{*}(y)=x$.
\end{defi}
...
$$
    \xymatrix{
    sSets \ar@<0.5ex>[r]_{u^{*}} \ar@<0.5ex>[rd]^{i_{!}}
    &pdSets \ar@<0.5ex>[l]_{u_{!}} \ar@<0.5ex>[d]_{v_{!}} \\
    &dSets \ar@<0.5ex>[lu]^{i^{*}} \ar@<0.5ex>[u]_{v^{*}}
    }
    \xymatrix{
    sSets \ar@<0.5ex>[r]_{u^{*}} \ar@<0.5ex>[rd]^{i_{*}}
    &pdSets \ar@<0.5ex>[l]_{u_{*}} \ar@<0.5ex>[d]_{v_{*}} \\
...
Tambi\'en extiende el nervio de categor\'ias a conjuntos simpliciales. Sea $\mathcal{C}$ una categor\'ia cualquiera y $P_{\mathcal{C}}$ es una op\'erada coloreada asociada, entonces
$$
    i^{*}(N_d(P_{\mathcal{C}})) = N(\mathcal{C})
$$
...
\begin{enumerate}
    \item[{\rm (i)}] $s(x_a) = \text{id}_{x_a} \in \tau_d(X)(x_a;a_a)$ si $x_a\in X_\eta$ y $s=\sigma^{*}$, donde $\sigma$ es la degenerada $\sigma\colon C_1 \to\eta$.
...
\end{enumerate}
Entonces, $\tau_d(\Omega[T])=\Omega(T)$ para todo \'arbol $T$ en $\Omega$.
...
La categor\'ia de los conjuntos dendroidales es una categor\'ia de prehaces, y por consecuencia es cerrada cartesianamente.
...
$$
\begin{defi}
    Para todo par de \'arboles $T$ y $S$ en $\Omega$, el \emph{producto tensorial} de los conjuntos de representables $\Omega[T]$ y $\Omega[S]$ se define como
    $$
    ...
    Esto define un producto tensorial en toda la categor\'ia de conjuntos dendroidales, ya que es una categor\'ia de prehaces y entonces cada objeto es un col\'imite can\'onico de representables y $\otimes$ mantiene los col\'imites en cada variable.
\end{defi}
\begin{defi}
