...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Function{generate\_shuffles}{$self$}
            \State $queue\leftarrow$ Append initial tree
            \State $dictionary\leftarrow$ Save initial tree
            \While{$queue$ not empty}
            \State $tree \leftarrow$ Pop the first tree in $queue$
            \For {$location$ in $tree.find\_percolations()$}
                \State $new\_tree \leftarrow$ Apply the percolation on $location$ of the $tree$
                \If {$new\_tree$ not in $dictionary$}
                    \State $queue\leftarrow new\_tree$ Append new tree
                    \State $dictionary\leftarrow$ Save new tree
                \EndIf
            \EndFor
            \EndWhile
            \State\Return $dictionary$
        \EndFunction
        \Ensure Returns $dictionary$ with all the shuffles found
    \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
    \caption{Pseudoc\'odigo del algoritmo para encontrar los v\'ertices para hacer un intercambio en un \'arbol $T$. Pod\'eis encontrar el c\'odigo completo en el anexo.}
    \begin{algorithmic}[1]
        \Require Requires the class instance of \emph{ShuffleLattice}, an optional tree $X$ and an empty array $found$
        \Function{find\_percolations}{$self$, $X$, $found$}
        \If{$X$ is empty}
            \State $X\leftarrow self.tree$ \Comment{Note that $self.tree$ refers to $T$ on top of $S$}
        \EndIf
        \If{$X.node$ is from $self.S.operations$}
            \For{$branch$ in $X.branches$} \Comment{Note that $branch$ is a Tree}
                \If{$branch.node$ not in $self.T.operations$}
                    \State\Break
                \EndIf
            \EndFor
            \ForElse \Comment{Note that this else only occurs when the \textbf{for} hasn't been broken}
                \State $found\leftarrow$ Append $x$
            \EndForElse
        \EndIf
        \For{$X_i$ in $X.branches$}
            $find\_percolations(self, X_i, found)$
        \EndFor
        \State\Return found
        \EndFunction
